...
const PlanPage: React.FC = () => {
  ...
  const handleDelete = async (id: string | null) => {
    try {
      ...
      const body = { id }
      const res = await Apiservice.postAuth(apiEndPoints.plan.delete, body, token);
  ...
  const getPlans = async () => {
    try {
      ...
      let url = `${apiEndPoints.plan.get}?search=${searchTerms ?? ""}&sortBy=${sorting[0]?.id ?? ""}&sortOrder=${sorting[0]?.desc ? "desc" : "asc"}&page=${pageState.pageIndex + 1}&perPage=${pageState.pageSize}`
